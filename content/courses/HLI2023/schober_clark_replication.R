...
conditions.url <- "https://docs.google.com/spreadsheets/d/1JEOcO8uqd1p2Pk_xKJHXuqbT87paeiERUoO0h7nVMcc/edit#gid=0"
condition.list <- read_sheet(conditions.url, sheet = "Sheet1")
...
for (eaf in eafs) {
  ...
  trial.intervals <- eaf.data %>%
    filter(grepl("[tT]rial_[nN]umber", tier_name)) %>%
  ...
  card.intervals <- eaf.data %>%
    filter(grepl("[cC]ard_[iI][dD]", tier_name)) %>%
  ...
  placement.intervals <- eaf.data %>%
    filter(grepl("[pP]lacement", tier_name)) %>%
...
interaction.data <- interaction.data %>%
  left_join(mutate(condition.list, student_name = tolower(`Last Name`),
                   condition = `Recruitment type`) %>%
              select(c(student_name, condition)))
...
trial.lengths.bystudent <- interaction.data %>%
  ...
  group_by(condition, student_name, trial_number) %>%
  summarize(
...
trial.lengths.overall.cond <- trial.lengths.bystudent %>%
...
ggplot(trial.lengths.overall.cond,
       aes(x = trial_number, y = mean.len, group = 1)) +
...
trial.durations.overall.cond <- trial.durations.bystudent %>%
  group_by(condition, trial_number) %>%
  summarize(
    mean.dur = mean(total.dur),
    median.dur = median(total.dur),
    sd.dur = sd(total.dur),
...
ggplot(trial.durations.overall.cond,
       aes(x = trial_number, y = mean.dur, group = 1)) +
  facet_grid(~ condition) +
  geom_text(data = trial.durations.bystudent, aes(x = trial_number,
                                                  y = total.dur,
                                                  group =student_name,
...
interaction.data.accuracy <- interaction.data.accuracy %>%
  filter(!(student_name %in% c("alberro", "johnson", "baker", "rodriguez", "sharma")))
...
accuracy.overall.cond <- accuracy.bystudent %>%
  group_by(condition, trial_number, placer) %>%
  summarize(
    mean.acc = mean(accuracy.prop),
    median.acc = median(accuracy.prop),
    sd.acc = sd(accuracy.prop),
...
ggplot(accuracy.overall.cond, aes(x = trial_number, y = mean.acc,
                             group = placer, linetype = placer, color = placer)) +
  facet_grid(~ condition) +
  coord_cartesian(ylim = c(0, 1)) +
  # ylim(0,1) +
...
acc.mdl <- lmer(accuracy.prop ~ condition + trial_number + placer + (1|student_name), data = accuracy.bystudent)
...
timing.data <-timing.data %>%
  ...
  left_join(mutate(condition.list, student_name = tolower(`Last Name`),
              ...
              select(c(student_name, condition))) %>%
  # BRITTLE
  filter(abs(time_diff_sec) <= 20)

timing.data.bystudent <- timing.data %>%
  group_by(condition, student_name, trial_number, placer) %>%
  summarize(
...
timing.data.overall.cond <- timing.data.bystudent %>%
  group_by(condition, trial_number, placer) %>%
  summarize(
    mean.td = mean(mean.time_diff),
    median.td = median(mean.time_diff),
...
## Other stuff
bigolestring <- interaction.data %>%
  filter(tier_name == "director_speech") %>%
  pull(content) %>%
  unlist() %>%
  paste0(collapse = " ")
word.table <- sort(table(unlist(strsplit(bigolestring, " "))),      # Create frequency table
                   decreasing = TRUE)
